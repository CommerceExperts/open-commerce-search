 ...
 */
@NoArgsConstructor
public class SkipDocumentDataProcessor implements DataPreProcessor {

	private Map<String, Pattern> filterPatterns = new HashMap<>();

	public void configure(IndexConfiguration config) {
		config.getDataProcessorConfiguration()
				.getConfiguration()
				.getOrDefault(this.getClass().getSimpleName(), Collections.emptyMap())
				.forEach((field, regex) -> filterPatterns.put(field, Pattern.compile(regex)));
	}
